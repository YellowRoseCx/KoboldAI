...
def prepare_3bit_load(modelpath):
    path_3bit = os.path.join(modelpath, "model.safetensors")
    if os.path.isfile(path_3bit):
        return path_3bit, False

    path_3bit = os.path.join(modelpath, "model.ckpt")
    if os.path.isfile(path_3bit):
        return path_3bit, False

    # Legacy format support
    paths_3bit = ["3bit*.safetensors", "3bit*.pt", "3bit-old.safetensors", "3bit-old.pt"]
    result = False
    groupsize = -1
    for p in paths_3bit:
        p = os.path.join(modelpath, p)
        val = [v for v in glob.glob(p) if "3bit-old" not in v]
        if val:
            result = val[0]
            fname = Path(result).parts[-1]
            g = re.findall("^(?:3bit)(?:-)(\\d+)(?:g-?)", fname)
            groupsize = -1
            if g:
                groupsize = int(g[0])
            break
...
def load_model_gptq_settings():
    ...
    gptq_legacy_files = glob.glob(os.path.join(utils.koboldai_vars.custmodpth, "3bit*.pt")) + glob.glob(os.path.join(utils.koboldai_vars.custmodpth, "3bit*.safetensors"))
    if "gptq_bits" in js:
        ...
        utils.koboldai_vars.gptq_model = True
        utils.koboldai_vars.gptq_bits = 3
...
class HFTorch3BitInferenceModel(HFTorchInferenceModel):
    ...
    def _get_model(self, location: str, tf_kwargs: Dict):
        ...
        path_3bit, legacy_groupsize = prepare_3bit_load(utils.koboldai_vars.custmodpth)

        if utils.koboldai_vars.gptq_version < 0:
            utils.koboldai_vars.gptq_version = get_gptq_version(path_3bit)
        ...
        logger.info(f"Using 3-bit file: {path_3bit}, type {utils.koboldai_vars.model_type}, version {utils.koboldai_vars.gptq_version}, groupsize {groupsize}")
        if utils.koboldai_vars.model_type == "gptj":
            model = load_quant_offload(gptj_load_quant, utils.koboldai_vars.custmodpth, path_3bit, 3, groupsize, self.gpu_layers_list)
        elif utils.koboldai_vars.model_type == "gpt_neox":
            model = load_quant_offload(gptneox_load_quant, utils.koboldai_vars.custmodpth, path_3bit, 3, groupsize, self.gpu_layers_list)
        elif utils.koboldai_vars.model_type == "llama":
            model = load_quant_offload(llama_load_quant, utils.koboldai_vars.custmodpth, path_3bit, 3, groupsize, self.gpu_layers_list)
        elif utils.koboldai_vars.model_type == "opt":
            model = load_quant_offload(opt_load_quant, utils.koboldai_vars.custmodpth, path_3bit, 3, groupsize, self.gpu_layers_list)
        elif utils.koboldai_vars.model_type == "mpt":
            model = load_quant_offload(mpt_load_quant, utils.koboldai_vars.custmodpth, path_3bit, 3, groupsize, self.gpu_layers_list)
        else:
            raise RuntimeError(f"3-bit load failed. Model type {utils.koboldai_vars.model_type} not supported in 3-bit")
